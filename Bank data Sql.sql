...
Alter table bank_data change Column `ï»¿ State Abbr`State_Abbr varchar(20);
...
FROM bank_data
WHERE (Recoveries) > 0 OR (Collection)> 0;
SELECT 
  COUNT(DISTINCT (Client_id)) AS Delinquent_Client_Count
FROM bank_data
...
SELECT 
  CAST(COUNT(CASE WHEN (Loan_Status) = 'Default' THEN 1 END) AS FLOAT) * 100 / 
